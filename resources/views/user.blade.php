            ...
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item dropdown">
                  <a href="#" class="nav-link dropdown-toggle icon-menu" data-toggle="dropdown" style="text-decoration: none">
                    ...
                    @php $user_unRead = \App\UserNotifications::where('notifiable_id', Auth::user()->id)->where('read_at', NULL)->orderBy('created_at','desc')->get(); @endphp
                      <span class="badge bg-danger">@php count($user_unRead) @endphp</span>
                  </a>
                  <ul class="dropdown-menu notifications">
                    @php $user_notifikasi = \App\UserNotifications::where('notifiable_id', Auth::user()->id)->orderBy('created_at','desc')->get(); @endphp
                    @forelse ($user_notifikasi as $notifikasi)
                      @php $notif = json_decode($notifikasi->data); @endphp
                      <li>
                        <a href="{{ route('order.all') }}" class="dropdown-item btnunNotif" data-num="{{--$loop->iteration--}}"><small>[{{ $notif->nama }}] {{ $notif->message }}</small></a>
                      </li>
                    @empty
                        <li>
                          <a href="#" class="dropdown-item btnunNotif" data-num="{{--$loop->iteration--}}" >
